...
def main(args):
    ...
    if not mode_name or mode_name == "none":
        ...
        query_processor = GQR(input_path=input_path, llm=llm, output_dir=output_dir, retriever_type=retriever_type)
    
    elif mode_name == "q2e":
        query_processor = Q2E(input_path=input_path, llm=llm, output_dir=output_dir, retriever_type=retriever_type)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", required=False, choices=sorted(MODE), help="Query reformulation method. Available options: {}.".format(", ".join(sorted(MODE))))
