...
class PrecisionR(Evaluator):

    def __init__(self, json_path, ground_truths:dict): # format of the ground truth is {question: [city, city, city], question: [city, city, city]}
        super().__init__(json_path, ground_truths)
        self.data = None
...
#         print(list(citydict.items())[j][0]) # gives the city
    ...
    def precision_at_r(self):
        ...
        for question, citydict in self.master.items():
            ...
            self.k = len(truthlist) # k = R => Precision at R
